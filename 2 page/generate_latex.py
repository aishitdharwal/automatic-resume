...
def Gen_Latex():
    ...
    a_a_data = pd.read_excel('../resume_data_2page.xlsx', sheet_name='a_a_data')
    ...
    # WORK EXP ----------------------------------------------------
    we_data = pd.read_excel('../resume_data_2page.xlsx', sheet_name='we_data')
    project_template = r"""\begin{itemize}[label={}, leftmargin=0cm, itemsep = -1mm]
        \item \textbf{{project_name}} \hfill \textbf{\uppercase{company_name}}
        \item \emph{position}\hfill\emph{date}\vspace*{-2mm}
    ...
    # PROJECTS ----------------------------------------------------
    project_data = pd.read_excel('../resume_data_2page.xlsx', sheet_name='project_data')
    project_template = r"""\begin{itemize}[label={}, leftmargin=0cm, itemsep = -1mm]
        \item \textbf{{project_name}} \hfill {{company_name}}
        \item \emph{position}\hfill\emph{date}\vspace*{-2mm}
        \begin{itemize}[itemsep = -0.4 mm, leftmargin=*, label=\tiny$\bullet$]
            \item {}
            \item {}
    ...
    por_data = pd.read_excel('../resume_data_2page.xlsx', sheet_name='por_data')
    por_template = r"""\begin{itemize}[label={}, leftmargin=0cm, itemsep = -1mm]
        \item \textbf{por} \hfill\emph{date}
        \item \emph{pordept}\hfill\emph{company_name}\vspace*{-2mm}
        \begin{itemize}[itemsep = -0.4 mm, leftmargin=*, label=\tiny$\bullet$]
            \item {}
            \item {}
            \item {}
    ...
    for n_por in range(len(df)):
        project = por_template.replace('pordept', df['pordept'][n_por])
        project = project.replace('company_name', df['company'][n_por])
        # project = project.replace('position', df['position'][n_por])
        project = project.replace('por', df['por'][n_por])
    ...
    tp_data = pd.read_excel('../resume_data_2page.xlsx', sheet_name='tp_data')
    ...
    kcu_data = pd.read_excel('../resume_data_2page.xlsx', sheet_name='kcu_data')
    ...
    # EXTRACURRICULARS
    ec_data = pd.read_excel('../resume_data_2page.xlsx', sheet_name='ec_data')
    ec_template = r"""\begin{itemize}[itemsep = -0.75 mm, leftmargin=*]
		...
		\end{itemize}
    """
    ec_template = ec_template.replace('itemsep = -0.75mm]', ('itemsep = ' + config.ec_itemsep_1 + ']'))
